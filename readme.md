...
전량을 [웹 지도](https://its-2026.vercel.app/)(`web/`)로 배포합니다. 지도에서는 도로 음영 지수 등급별 도로 색상, 지점 선택 시 스트리트뷰 원본·세그멘테이션·깊이·BEV 진단 이미지, 파이프라인 판(version) 전환, TAAS 원시 사고지점 오버레이(사망/중상·연도별 토글), 기존 자율주행 버스 노선별 평균 지수, 버스 정류장 오버레이(강릉시 BIS ID·공식 좌표, 지도 범위 안 431개소), 수요 시나리오의 두 끝점(강릉역·강릉단오제전수교육관), 그리고 **α 슬라이더**를 확인할 수 있습니다. 기존 제안 노선 1·2·3의 데이터와 지도 표시는 제거했으며, 향후 노선을 추가할 수 있는 기능 틀은 유지합니다.

강릉시 BIS 등록 노선 132개(중단·폐지 표기 포함)를 선택하면 전체 경로와 경유 정류장을 표시합니다. 2025년 전체(365일)와 단오제 기간(8일)의 승하차를 일평균으로 표시하며, 연간·단오제 승하차량 및 두 기간의 일평균 차이로 정렬할 수 있습니다. 선 색상은 정류장별 **순승차량(승차−하차)**을 주변 300m에서 거리 가중 보간하고, 선택 노선·기간의 정류장별 순승차량 최대 절댓값으로 정규화한 뒤 제곱근으로 색 강도를 조절합니다. 양수는 빨강, 음수는 파랑, 0은 중립색, 주변 기록이 없으면 회색입니다. 이는 실제 차내 재차 인원이 아니며, 현재 BIS 경로에 2025년 수요를 연결한 결과입니다. 툴팁에는 정류장의 전체 노선 합계와 선택 노선의 승하차 일평균을 함께 표시합니다.
